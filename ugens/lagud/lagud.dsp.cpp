...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		fConst0 = (1.0 / min(192000.0, max(1.0, double(fSamplingFreq))));
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("up", &fEntry0, 0.10000000000000001, 0.0, 9999.0, 0.0001);
		ui_interface->declare(&fEntry1, "2", "");
		ui_interface->addNumEntry("down", &fEntry1, 0.10000000000000001, 0.0, 9999.0, 0.0001);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		double fSlow0 = exp((0.0 - (fConst0 / double(fEntry0))));
		double fSlow1 = exp((0.0 - (fConst0 / double(fEntry1))));
		for (int i = 0; (i < count); i = (i + 1)) {
			double fTemp0 = double(input0[i]);
			double fTemp1 = ((fRec0[1] > fTemp0)?fSlow1:fSlow0);
			fRec1[0] = (((1.0 - fTemp1) * fTemp0) + (fTemp1 * fRec1[1]));
			fRec0[0] = fRec1[0];
	...
	FAUST_ADDNUMENTRY("up", fEntry0, 0.10000000000000001, 0.0, 9999.0, 0.0001);
	FAUST_ADDNUMENTRY("down", fEntry1, 0.10000000000000001, 0.0, 9999.0, 0.0001);
#endif
